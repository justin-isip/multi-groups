...
# How many studies in each?
length(unique(source_multiple$SS)) # 153
length(unique(ss_multiple$SS)) # 106

# Rows that are different between each? 
diff <- setdiff(source_multiple, ss_multiple) # 133369 rows

# Have a look at orders sampled at the source level
source_multiple %>% dplyr::select(Source_ID, SS, Order, Sampling_method, Site_name, Habitat_as_described) %>% unique() %>% View()

a <- source_multiple %>% dplyr::select(Source_ID, SS, Order, Sampling_method) %>% unique()
