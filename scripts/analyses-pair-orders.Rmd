...
# Testing influence.ME and robustlmm 
```{r}
...
outliers <- diptera_hymenoptera[diptera_hymenoptera$weights != 1,]

# Print
print(outliers)

# Hist of potential outliers
hist(diptera_hymenoptera$weights, main = "histogram of robust weights", xlab = "weights")

# Simplify
outliers <- outliers %>% dplyr::select(Source_ID, SS, Order, weights) 

# Average robustness weight per study
robust_weights <- outliers %>% group_by(SS) %>% summarise(mean(weights)) 

# Extract cooks weights
cooks_weights <- as.data.frame(cooks.distance(dip_col_influence, sort = TRUE))

# Extract studies
cooks_weights$SS <- rownames(cooks_weights)

# Reorder cols
cooks_weights <- cooks_weights[, c("SS", "V1")]

# Rename col
cooks_weights <- rename(cooks_weights, "Cooks weights" = "V1")

# Merge robust weights to cooks weights
combined <- inner_join(robust_weights, cooks_weights, by = "SS")

# Rename col
combined <- rename(combined, "Robust weights" = "mean(weights)")

# Plot the robust weights against the cooks weights
plot(combined$`Robust weights`, combined$`Cooks weights`)

# Robust lmer model
r_m2 <- rlmer(logAbundance ~ Predominant_land_use * Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = diptera_hymenoptera)

# Influence is the workhorse function of the influence.ME package, Based on a priorly estimated
# mixed effects regression model (estimated using lme4), the influence() function iteratively modifies
# the mixed effects model to neutralize the effect a grouped set of data has on the parameters, and
# which returns the fixed parameters of these iteratively modified models. These are used to
# compute measures of influential data.

dip_col_influence <- influence(r_m2, "SS") # based on the robust model

# Calculate cooks distance for each of the studies
cooks.distance(dip_col_influence, sort = TRUE)

# Calculate % change for each of the parameters for each of the studies, sort based on a column 
pchange(dip_col_influence, parameters = 1:2, sort= T, to.sort="Predominant_land_useYoung secondary vegetation", abs=FALSE)
...
sigtest(dip_col_influence, test = 1.96, parameters = 0, sort = FALSE, to.sort = NA)

# ^ the above gives the t statistic after deletion of each evaluated case, whether or not this new t statistic is statistically significant based on 1.96, and
# whether or not this is significantly different from the original model. In other words, if a parameter was significant in the original model, but is no longer
# significant after the deletion of a specific case in the model, this would be specified in the last column. 

# Plot predictors
plot_model(dhy_m2, type = "pred", show.data = T)
plot_model(r_m2, type = "pred", show.data = T)
...
# Performance package
check_model(dhy_m2)
model_performance(dhy_m2)

```

# Dummy data
```{r}
# Set the number of rows, mean and sd for the dummy data
n <- 1000
mean <- 100
sd <- 1
...
dummy <- data.frame(
  Order = sample(c("Diptera", "Hymenoptera"), n, replace = TRUE), # single factor fixed effect with two levels
  Abundance = rnorm(n, mean, sd), # based on above
  Study = sample(paste0("Study", seq_len(4))) # Add 4 studies (random effects)
)
...
dummy <- 
  dummy %>%
  group_by(Study) %>%
  mutate(Abundance = ifelse(Study == "Study1", rnorm(n, mean = 100, sd = 20), 
                            rnorm(n, mean = 100, sd = 1)))

# Have a look at the distribution of abundance values based on study
ggplot(dummy, aes(x = Abundance, fill = Study)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 10)

# Make study a factor
dummy$Study <- as.factor(dummy$Study)

# Run the simple model
dum_1 <-lmer(Abundance ~ Order + (1 | Study), data = dummy)

# Robust model
dum_2 <- rlmer(Abundance ~ Order + (1 | Study), data = dummy)

# Extract the robustness weights from the robust lmm
weights <- getME(dum_2, "w_e") 

# Re-add them to the initial df
dummy$weights <- weights

# Have a look at the distribution of the weights
ggplot(dummy, aes(x = weights, fill = Study)) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 10)

# Extract the potential outliers, i.e. those weights not equal to 1
outliers <- dummy[dummy$weights != 1,]

# Average robustness weight per study across all obs
robust_weights <- outliers %>% group_by(Study) %>% summarise(mean(weights)) 

# Dummy influence
dummy_influence <- influence(dum_2, "Study") # based on the robust model

# Extract cooks weights
cooks_weights <- as.data.frame(cooks.distance(dummy_influence, sort = TRUE)) # Study 1 has the highest cooks distance

# Extract studies
cooks_weights$Study <- rownames(cooks_weights)

# Reorder cols
cooks_weights <- cooks_weights[, c("Study", "V1")]

# Rename col
cooks_weights <- rename(cooks_weights, "Cooks weights" = "V1")

# Merge robust weights to cooks weights
combined <- inner_join(robust_weights, cooks_weights, by = "Study")
...
# STUDY 1 HAS LOWEST ROBUSTNESS WEIGHTS AND HIGHEST COOKS DISTANCE AS EXPECTED!
```
...
# Robust model
dip_col_robust <- rlmer(logAbundance ~ Predominant_land_use * Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = diptera_coleoptera)

# Calculate the influences of each study
dip_col_influence <- influence(dip_col_robust, "SS") # based on the robust model
...
weights <- weights(dip_col_robust)

diptera_coleoptera$weights <- weights

outliers <- diptera_coleoptera[diptera_coleoptera$weights != 1,]

print(outliers)

diptera_coleoptera %>% dplyr::select(Source_ID, SS, Order, weights) %>% filter(weights != 1) %>% View()
...
# Robust model
dip_hemi_robust <-rlmer(logAbundance ~ Predominant_land_use * Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
...
# Calculate cooks distance for each of the studies
cooks.distance(dip_hemi_influence, sort = TRUE)
