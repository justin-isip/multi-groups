...
# Combine cooks weights (abundance)
```{r}
...
dip_hym <- data.frame(Study = rownames(cooks_weights_dip_hym), dip_hym_cooks = cooks_weights_dip_hym[,1])
col_lep <- data.frame(Study = rownames(cooks_weights_col_lep), col_lep_cooks = cooks_weights_col_lep[,1])
dip_col <- data.frame(Study = rownames(cooks_weights_dip_col), dip_col_cooks = cooks_weights_dip_col[,1])
dip_hemi <- data.frame(Study = rownames(cooks_weights_dip_hemi), dip_hemi_cooks = cooks_weights_dip_hemi[,1])
dip_lep <- data.frame(Study = rownames(cooks_weights_dip_lep), dip_lep_cooks = cooks_weights_dip_lep[,1])
hemi_col <- data.frame(Study = rownames(cooks_weights_hemi_col), hemi_col_cooks = cooks_weights_hemi_col[,1])
hemi_lep <- data.frame(Study = rownames(cooks_weights_hemi_lep), hemi_lep_cooks = cooks_weights_hemi_lep[,1])
hym_hemi <- data.frame(Study = rownames(cooks_weights_hym_hemi), hym_hemi_cooks = cooks_weights_hym_hemi[,1])
hym_lep <- data.frame(Study = rownames(cooks_weights_hym_lep), hym_lep_cooks = cooks_weights_hym_lep[,1])
hym_col <- data.frame(Study = rownames(cooks_weights_hym_col), hym_col_cooks = cooks_weights_hym_col[,1])

list_of_dfs <- list(dip_hym, col_lep, dip_col, dip_hemi,
                       dip_lep, hemi_col, hemi_lep, hym_hemi,
                       hym_lep, hym_col)

combined <- reduce(list_of_dfs, full_join, by ="Study")

combined[] <- lapply(combined, function(x) if(is.numeric(x)) round(x, digits= 4) else x)

write.csv(combined, "cooks_distances_ab.csv", row.names = TRUE)
...
# Combine cooks weights (species richness)
```{r}

dip_hym_sr <- data.frame(Study = rownames(cooks_weights_dip_hym_sr), dip_hym_cooks = cooks_weights_dip_hym_sr[,1])
hym_col_sr <- data.frame(Study = rownames(cooks_weights_hym_col_sr), hym_col_cooks = cooks_weights_hym_col_sr[,1])
dip_col_sr <- data.frame(Study = rownames(cooks_weights_dip_col_sr), dip_col_cooks = cooks_weights_dip_col_sr[,1])
hemi_col_sr <- data.frame(Study = rownames(cooks_weights_hemi_col_sr), hemi_col_cooks = cooks_weights_hemi_col_sr[,1])
col_lep_sr <- data.frame(Study = rownames(cooks_weights_col_lep_sr), col_lep_cooks = cooks_weights_col_lep_sr[,1])
hym_hemi_sr <- data.frame(Study = rownames(cooks_weights_hym_hemi_sr), hym_hemi_cooks = cooks_weights_hym_hemi_sr[,1])
hym_lep_sr <- data.frame(Study = rownames(cooks_weights_hym_lep_sr), hym_lep_cooks = cooks_weights_hym_lep_sr[,1])
dip_hemi_sr <- data.frame(Study = rownames(cooks_weights_dip_hemi_sr), dip_hemi_cooks = cooks_weights_dip_hemi_sr[,1])
dip_lep_sr <- data.frame(Study = rownames(cooks_weights_dip_lep_sr), dip_lep_cooks = cooks_weights_dip_lep_sr[,1])
hemi_lep_sr <- data.frame(Study = rownames(cooks_weights_hemi_lep_sr), hemi_lep_cooks = cooks_weights_hemi_lep_sr[,1])


list_of_dfs <- list(dip_hym_sr, hym_col_sr, dip_col_sr, hemi_col_sr,
                       col_lep_sr, hym_hemi_sr, hym_lep_sr, dip_hemi_sr,
                       dip_lep_sr, hemi_lep_sr)

combined_sr <- reduce(list_of_dfs, full_join, by ="Study")

combined_sr[] <- lapply(combined_sr, function(x) if(is.numeric(x)) round(x, digits= 4) else x)

write.csv(combined_sr, "cooks_distances_sr.csv", row.names = TRUE)
