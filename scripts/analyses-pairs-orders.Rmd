...
five_orders$Multiple_groups <- as.factor(five_orders$Multiple_groups)


levels(five_orders$Multiple_groups)

# Now insects only contains studies that sample more than one group of insects
...
studies <-
  ...
  mutate(
    Multiple_groups = 
      case_when(
        (length(unique(Higher_taxon))) == 2 ~ "2",
        (length(unique(Higher_taxon))) == 2 ~ "2",
        (length(unique(Higher_taxon))) == 3 ~ "3",
        (length(unique(Higher_taxon))) == 4 ~ "4",
        (length(unique(Higher_taxon))) == 5 ~ "5")) %>% 
        ungroup() %>%
        droplevels() 


studies[studies$SS == "AD1_2008__Billeter 15",]

# 784 rows for studies that do two groups
two_ss <- studies %>% filter(Multiple_groups == "2") 
# 1194 rows for studies that do three groups
three_ss <- studies %>% filter(Multiple_groups == "3") 
# 1500 rows for studies that do four groups
four_ss <- studies %>% filter(Multiple_groups == "4") 
# 2895 rows for studies that do all 5 groups
five_ss <- studies %>% filter(Multiple_groups == "5") 
...
z[z$SS == "AD1_2008__Billeter 15",]


levels(studies$Multiple_groups) 

is.na(studies$Higher_taxon)


b <- studies[!complete.cases(studies$Higher_taxon),]



# 3 ways to do the same thing
model_data_ab[model_data_ab$SS == "AD1_2007__Ockinger 1",]
subset(model_data_ab, SS == "AD1_2007__Ockinger 1")
a <- model_data_ab %>% filter(SS == "AD1_2007__Ockinger 1")
