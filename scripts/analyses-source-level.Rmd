...
source_only <- anti_join(source_multiple, ss_multiple, by = c("Source_ID", "SS", "Order", "Sampling_method", "Site_name", "Habitat_as_described"))
...
source_only <- source_only %>%
  ...
  dplyr::select(Source_ID, SS, Order, Sampling_method, site_habitat_combo) %>% # Select the columns of interest
  unique()
  
# Simplify the study level df
studies_only <- ss_multiple %>%
  mutate(site_habitat_combo = paste(Site_name, Habitat_as_described)) %>% # Concatenates 'Site_name' and 'Habitat_as_described'
  dplyr::select(Source_ID, SS, Order, Sampling_method, site_habitat_combo) %>%
  unique()
...
same_site_habitat <- inner_join(studies_only, source_only, by = ("site_habitat_combo")) 
...
# The unique studies in this column are the studies which have the same sites and habitats 
