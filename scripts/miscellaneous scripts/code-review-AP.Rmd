...
#### This markdown is a shortened version of the analysis for my first chapter. It first subsets multiple insect groups at the source rather than study level and then merges the two to have an overall multi-group dataset. I then update the sampling methods for those studies which have obscure sampling methods in the PREDICTS database. I then filter all of the PREDICTS methods for non-bycatch data based on both the expert survey and personal comms with entomologists. I then prepare the model data and calculate the site metrics. I then run the abundance and species richness models for two comparisons and also calculate the interaction importance using glmm.hp() and also run through the code for creating the plots based on the model outputs.
...
source("site_comparison_functions.R")
...
```{r}
diversity <- readRDS("/Users/andyp/R-projects/Isip_Multigroups/data/diversity-2025-05-02-02-32-27.rds")
```
...
saveRDS(clean, "diversity-2025-12-31-00-00-00.rds") # Saving the file to feed into pipe-line.
# File name chosen so as to not upset later checks of name and date.
...
#### Create the plot, with the fits on the log scale, setting primary to zero and plotting the coefficients for each of the land uses as departures from primary.
```{r}
hymenoptera_coleoptera_plot_ab <- combined %>%
    ...
    geom_linerange(aes(ymin = Upper_ci, ymax = Lower_ci), position = position_dodge(width = 0.5), linewidth = 2)+
    theme_classic()+
 ...
 # this overrides the colours for the groups as above
    guides(color = guide_legend(
      override.aes=list(shape = 16, size = 10, linetype = 0))) +
        scale_y_continuous(breaks = scales::pretty_breaks(n = 8)) +
  theme(legend.title = element_text(face = "bold", size = 16),
...
#### Tidy
```{r}
rm(col_eff, combined, hc_m1, hym_eff, hym_m1)
```

#### Interaction importance for Hym vs Col
```{r}

hc_m1 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
              data = hymenoptera_coleoptera)

# glmm.hp does not like the * symbol in the model syntax

# Run the hierarchical partitioning
glmm.hp(hc_m1)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0141/(0.0032+0.0141)

# Interaction importance = 0.82

# relevel model with Hymenoptera as reference level just to sense check
hymenoptera_coleoptera <- within(hymenoptera_coleoptera, Higher_taxon <- relevel(Higher_taxon, ref = "Hymenoptera"))

# maximal model with Hymenoptera as ref
hym_m1 <- lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
    data = hymenoptera_coleoptera)

glmm.hp(hym_m1)

# values are identical with the relevelled model, no need to do it for future models. 
```


#### Tidy
```{r}
rm(hc_m1, hymenoptera_coleoptera)
```
...
#ggsave("../figures/log_figures_bycatch_removed/col_hym.png", col_hym, height = 10, width = 10)
```

#### Tidy
```{r}
rm(col_hym, combined, hc_m2, hym_eff, hym_m1, hymenoptera_coleoptera, hymenoptera_coleoptera_plot_sr, hymenoptera_coleoptera_plot_ab, col_eff)
```
...
#### Tidy
```{r}
rm(combined, dhy_m2, dip_eff, hym_eff, hym_m1)
```

#### Interaction importance
```{r}
dhy_m2 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = diptera_hymenoptera)
...
# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0465/(0.0020+0.0465)

# Interaction importance = 0.95
```

#### Tidy
```{r}
rm(dhy_m2, diptera_hymenoptera)
```
...
#ggsave("../figures/log_figures_bycatch_removed/dip_hym.png", dip_hym, height = 10, width = 10)
dip_hym
```

#### Tidy
```{r}
rm(combined, dip_eff, dip_hym, diptera_hymenoptera_plot_ab, diptera_hymenoptera_plot_sr, hd_m2, hym_eff, hym_m1, hymenoptera_diptera)

model_data_ab %>% dplyr::select(SS, SSBS, SSBST, Order, Sampling_method, logAbundance) %>%  View()
```
