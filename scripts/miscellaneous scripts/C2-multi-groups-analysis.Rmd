...
# correlation (unconstrained)
cor.test(coleoptera, hymenoptera, method = "pearson")
...
hc_m2_sim <- simulateResiduals(hc_m2, n = 1000)
plot(hc_m2_sim) ; plot(nb1_sim)
```
...
hemiptera_hymenoptera_plot_ab <- combined %>%
    ...
    scale_colour_manual(values=group_colours) + # this overrides the colours for the groups as above
    guides(color = guide_legend(
      override.aes=list(shape = 16, size = 8,  linetype = 0))) +
        scale_y_continuous(breaks = scales::pretty_breaks(n = 8)) +
  theme(legend.title = element_text(face = "bold", size = 16),
        legend.text = element_text(size = 16)) 
...
ggsave("../figures/log_figures_bycatch_removed/hemi_hym.png", hemi_hym, height = 10, width = 10)
```
...
ggsave("../figures/log_figures_bycatch_removed/hym_lep.png", hym_lep, height = 10, width = 10)
```
...
ggsave("../figures/log_figures_bycatch_removed/col_dip.png", col_dip, height = 10, width = 10)
```
...
ggsave("../figures/log_figures_bycatch_removed/col_hemi.png", col_hemi, height = 10, width = 10)
```
...
ggsave("../figures/log_figures_bycatch_removed/col_lep.png", col_lep, height = 10, width = 10)
```
...
ggsave("../figures/log_figures_bycatch_removed/dip_hemi.png", dip_hemi, height = 10, width = 10)
```
...
ggsave("../figures/log_figures_bycatch_removed/dip_lep.png", dip_lep, height = 10, width = 10)
```
...
ggsave("../figures/log_figures_bycatch_removed/hemi_lep.png", hemi_lep, height = 10, width = 10)
```
...
n <- 5 # sv, plantation, pasture, cropland, urban
...
# For your comment on calculating the threshold r for significance at p < 0.05 for the correlation coefficients (of either 4 or 5 data points depending on whether not plantation is included) and the unconstrained correlation costing 2 df, whereas through the origin costing only 1 df. Does this code look sensible?
...
# Create the dataframe (unconstrained, abundance and SR)
df <- data.frame(
  Coleoptera = c(0,-0.90,-0.80,0.71,-0.97),
  Diptera = c(0.18, 0,-0.99,-0.88,-0.96),
  Hemiptera = c(-0.874,-0.98, 0,-0.98,-0.79),
  Hymenoptera = c(-0.076,-0.85,-0.96, 0,-0.95),
  Lepidoptera = c(-0.72,-0.97, -0.77,-0.95, 0),
  row.names = c("Coleoptera", "Diptera", "Hemiptera", "Hymenoptera", "Lepidoptera")
)
