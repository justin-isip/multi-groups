...
# Overview
This document creates the supplementary figures
...
#### Create the plot (log abundance values including primary).
...
```{r}
    ...
    geom_linerange(aes(ymin = Lwr_added, ymax = Upper_added), position = position_dodge(width = 0.5), linewidth = 2)+
    theme_classic()+
...
#### Tidy
```{r}
rm(col_eff, combined, hc_m1, hym_eff, hym_m1, hymenoptera_coleoptera)
```
...
#### Create the plot (species richness)
```{r}
...
ggsave("../figures/supp_figures/col_hym.png", col_hym, height = 10, width = 10)
```

#### Tidy
```{r}
rm(col_eff, col_hym, combined, hc_m2, hym_eff, hym_m1, hymenoptera_coleoptera_plot_ab, hymenoptera_coleoptera_plot_sr, hymenoptera_coleoptera)
```
...
```{r}
  ...
  dip_eff <- dip_eff %>% 
    mutate(
      Log_value = dip_eff$median + ifelse(row_number() != 1, dip_eff$median[1], 0),
      Upper_added = dip_eff$Upper_ci + ifelse(row_number() != 1, dip_eff$Upper_ci[1], 0), 
      Lwr_added = dip_eff$Lower_ci + ifelse(row_number() != 1, dip_eff$Lower_ci[1], 0)
    ) 
...
#### Create the plot
```{r}
    hymenoptera_diptera_plot_ab <- combined %>%
    ggplot()+
    aes(x = Predominant_land_use, y = Log_value, colour = Order, group = Order)+
    geom_point(size = 6, position = position_dodge(width = 0.5))+
    geom_linerange(aes(ymin = Lwr_added, ymax = Upper_added), position = position_dodge(width = 0.5), linewidth = 2)+
...
hymenoptera_diptera_plot_ab 
```

#### Tidy
```{r}
rm(combined, dhy_m2, dip_eff, diptera_hymenoptera, hym_eff, hym_m1)
```
...
#### Arrrange two plots so they abbut and save them
```{r}
dip_hym <- ggarrange(hymenoptera_diptera_plot_ab, diptera_hymenoptera_plot_sr, nrow = 2, common.legend =  TRUE, legend = "bottom") 
ggsave("../figures/supp_figures/dip_hym.png", dip_hym, height = 10, width = 10)
```

#### Tidy
```{r}
rm(combined, dip_eff, dip_hym, hymenoptera_diptera, hd_m2, hym_eff, hym_m1, hymenoptera_diptera_plot_ab, diptera_hymenoptera_plot_sr)
```
...
```{r}
  ...
  hemi_eff <- hemi_eff %>% 
    mutate(
      Log_value = hemi_eff$median + ifelse(row_number() != 1, hemi_eff$median[1], 0),
      Upper_added = hemi_eff$Upper_ci + ifelse(row_number() != 1, hemi_eff$Upper_ci[1], 0), 
      Lwr_added = hemi_eff$Lower_ci + ifelse(row_number() != 1, hemi_eff$Lower_ci[1], 0)
    ) 
...
#### Tidy
```{r}
rm(combined, hemi_eff, hh_m1, hym_eff, hym_m1, hymenoptera_hemiptera)
```
...
ggsave("../figures/supp_figures/hemi_hym.png", hemi_hym, height = 10, width = 10)
```

#### Tidy
```{r}
rm(combined, hemi_eff, hemi_hym, hemiptera_hymenoptera_plot_ab, hemiptera_hymenoptera_plot_sr, hh_m3, hym_eff, hym_m1, hymenoptera_hemiptera)
```
...
lep_m1 <- lmer(logAbundance ~ Predominant_land_use * Higher_taxon + (1 | Source_ID) + (1 | SS), 
  ...
  lep_eff <- lep_eff %>% 
    mutate(
      Log_value = lep_eff$median + ifelse(row_number() != 1, lep_eff$median[1], 0),
      Upper_added = lep_eff$Upper_ci + ifelse(row_number() != 1, lep_eff$Upper_ci[1], 0), 
      Lwr_added = lep_eff$Lower_ci + ifelse(row_number() != 1, lep_eff$Lower_ci[1], 0)
    ) 
...
#### Tidy
```{r}
rm(combined, hl_m2, hym_eff, hymenoptera_lepidoptera, lep_eff, lep_m1)
```
...
ggsave("../figures/supp_figures/hym_lep.png", hym_lep, height = 10, width = 10)
```

#### Tidy
```{r}
rm(combined, hl_m2, hym_eff, hym_lep, hymenoptera_lepidoptera, hymenoptera_lepidoptera_plot_ab, hymenoptera_lepidoptera_plot_sr, lep_eff, lep_m1)
```
...
```{r}
  ...
  col_eff <- col_eff %>% 
    mutate(
      Log_value = col_eff$median + ifelse(row_number() != 1, col_eff$median[1], 0),
      Upper_added = col_eff$Upper_ci + ifelse(row_number() != 1, col_eff$Upper_ci[1], 0), 
      Lwr_added = col_eff$Lower_ci + ifelse(row_number() != 1, col_eff$Lower_ci[1], 0)
    ) 
...
#### Tidy
```{r}
rm(cd_m2, col_eff, coleoptera_diptera, combined, dip_eff, dip_m1)
```
...
ggsave("../figures/supp_figures/col_dip.png", col_dip, height = 10, width = 10)
```

#### Tidy
```{r}
rm(cd_m2, col_dip, col_eff, coleoptera_diptera, coleoptera_diptera_plot_ab, coleoptera_diptera_plot_sr, combined, dip_eff, dip_m1)
```
...
#### Tidy
```{r}
rm(ch_m1, col_eff, coleoptera_hemiptera, combined, hemi_eff, hemi_m1)
```
...
ggsave("../figures/supp_figures/col_hemi.png", col_hemi, height = 10, width = 10)
```

#### Tidy
```{r}
rm(ch_m1, col_eff, col_hemi, coleoptera_hemiptera, coleoptera_hemiptera_plot_ab, coleoptera_hemiptera_plot_sr, combined, hemi_eff, hemi_m1)
```
...
#### Tidy
```{r}
rm(cl_m2, col_eff, coleoptera_lepidoptera, combined, lep_eff, lep_m1)
```
...
ggsave("../figures/supp_figures/col_lep.png", col_lep, height = 10, width = 10)
```

#### Tidy
```{r}
rm(cl_m1, col_eff, col_lep, coleoptera_lepidoptera_plot_ab, coleoptera_lepidoptera_plot_sr, coleoptera_lepidoptera, combined, lep_eff, lep_m1)
```
...
#### Tidy
```{r}
rm(combined, dh_m1, dip_eff, diptera_hemiptera, hemi_eff, hemi_m1)
```
...
ggsave("../figures/supp_figures/dip_hemi.png", dip_hemi, height = 10, width = 10)
```

#### Tidy
```{r}
rm(combined, dh_m3, dip_eff, dip_hemi, diptera_hemiptera, diptera_hemiptera_plot_ab, diptera_hemiptera_plot_sr, hemi_eff, hemi_m1)
```
...
#### Tidy
```{r}
rm(combined, dip_eff, diptera_lepidoptera, dl_m3, lep_eff, lep_m1)
```
...
ggsave("../figures/supp_figures/dip_lep.png", dip_lep, height = 10, width = 10)
```

#### Tidy
```{r}
rm(combined, dip_eff, dip_lep, diptera_lepidoptera, diptera_lepidoptera_plot_ab, diptera_lepidoptera_plot_sr, dl_m3, lep_eff, lep_m1)
```
...
#### Tidy
```{r}
rm(combined, hemi_eff, hemiptera_lepidoptera, hl_m3, lep_eff, lep_m1)
```
...
ggsave("../figures/supp_figures/hemi_lep.png", hemi_lep, height = 10, width = 10)
```

#### Tidy
```{r}
rm(combined, hemi_eff, hemi_lep, hemiptera_lepidoptera, hemiptera_lepidoptera_plot_ab, hemiptera_lepidoptera_plot_sr, hl_m3, lep_eff, lep_m1)
```
