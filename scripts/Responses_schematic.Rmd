...
#### This markdown runs the bycatch filtering and then creates a blank plot for the schematic example of opposite versus similar responses comparing Hymenoptera and Coleoptera for the manuscript.
...
hym_m1 <- lmer(logAbundance ~ Predominant_land_use * Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
  ...
  combined
    ...
    # Get the phylopics
beetle <- pick_phylopic(name = "Lucanus cervus", n = 5, view = 5)    
wasp <- pick_phylopic(name = "Vespula alascensis", n = 5, view = 5)

    # Create the plot to pull the phylopics
opposite_responses_schematic %>%
    ggplot()+
    aes(x = Predominant_land_use, y = median, colour = Order, group = Order)+
    geom_hline(yintercept = 0, linewidth = 0.5, color = ("black"), linetype = 1)+
    geom_phylopic(img = beetle, height = 0.1) +
    #geom_phylopic(img = wasp, height= 0.1) +
    theme_classic()+
  ...
  # Blank plot for creating in powerpoint
blank_plot_ab <- same_responses_schematic %>%
    ggplot()+
    aes(x = Predominant_land_use, y = median, colour = Order, group = Order)+
    geom_hline(yintercept = 0, linewidth = 0.5, color = ("black"), linetype = 1)+
    #geom_point(size = 0, position = position_dodge(width = 0.5))+
    geom_linerange(aes(ymin = Upper_ci, ymax = Lower_ci), position = position_dodge(width = 0.5), linewidth = 0)+
    theme_classic()+
    scale_x_discrete(limits=comb_plot_limits)+
    geom_vline(xintercept = 1.5, linetype = 2)+
    geom_vline(xintercept = 2.5, linetype = 2)+
...
ggsave("../figures/blank_plot_ab.pdf", blank_plot_ab, height = 10, width = 14)
