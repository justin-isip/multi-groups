...
# This Rmd is looking at low taxonomic precision within the multi-order dataset, i.e. taxa not resolved to species level and potentially bycatch, the aim is to see how much data is poorly resolved. 
...
```{r}
five_orders %>% dplyr::select(Source_ID, SS, Sampling_method, Order, Genus, Species, Taxon_name_entered, Best_guess_binomial) %>% unique() %>% View()

colSums(five_orders[, c("Genus", "Species")] == "")
