...
#### This markdown is for calculating the importance of the LUI x Order interaction using the glmm.hp package
...
library(glmm.hp) # for calculating importance of the LUI x Order interaction
```
...
# values are the same with the releveled model, no need to do it for future models. 
```

#### Tidy
```{r}
rm(hc_m1, hym_m1, hymenoptera_coleoptera)
```
...
#### Hierarchical partitioning 
```{r}
hc_m2 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = hymenoptera_coleoptera)
...
# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0140/(0.0058+0.0140)

# Interaction importance = 0.71

```

#### Tidy
```{r}
rm(hc_m2, hymenoptera_coleoptera)
```
...
#### Hierarchical partitioning
```{r}
hd_m2 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
...
# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0406/(0.0015+0.0406)

# Interaction importance = 0.96
```

#### Tidy
```{r}
rm(hymenoptera_diptera, hd_m2)
```
...
#### Hierarchical partitioning
```{r}
hh_m1 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
...
# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0442/(0.0007+0.0442)

# Interaction importance = 0.98
```

#### Tidy
```{r}
rm(hymenoptera_hemiptera, hh_m1)
```
...
#### Hierarchical partitioning
```{r}
hh_m3 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID),
    data = hymenoptera_hemiptera)

glmm.hp(hh_m3)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0415/(0.0003+0.0415)
...
#### Tidy 
```{r}
rm(hh_m3, hymenoptera_hemiptera)
```
...
#### Hierarchical partitioning
```{r}
hl_m2 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = hymenoptera_lepidoptera)

glmm.hp(hl_m2)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0213/(0.0021+0.0213)

# Interaction importance = 0.91
```

#### Tidy 
```{r}
rm(hl_m2, hymenoptera_lepidoptera)
```
...
#### Hierarchical partitioning
```{r}
hl_m2 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = hymenoptera_lepidoptera)

glmm.hp(hl_m2)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0297/(0.0029+0.0297)
...
cd_m2 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = coleoptera_diptera)

glmm.hp(cd_m2)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0244/(0.0027+0.0244)

# Interaction importance = 0.90
```
...
cd_m2 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
    coleoptera_diptera)

glmm.hp(cd_m2)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0341/(0.0005+0.0341)
...
# maximal model
ch_m1 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
              data = coleoptera_hemiptera)

glmm.hp(ch_m1)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0383/(0.0031+0.0383)
...
# maximal model
ch_m1 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
              data = coleoptera_hemiptera)

glmm.hp(ch_m1)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0237/(0.0011+0.0237)
...
cl_m2 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS),
    data = coleoptera_lepidoptera)

glmm.hp(cl_m2)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.1400/(0.0043+0.1400)
...
#### Tidy
```{r}
rm(cl_m2, coleoptera_lepidoptera)
```
...
# maximal model
cl_m1 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
              data = coleoptera_lepidoptera)

glmm.hp(cl_m1)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.1816/(0.0050+0.1816)
...
#### Tidy
```{r}
rm(cl_m1, coleoptera_lepidoptera)
```
...
# maximal model
dh_m1 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID) + (1 | SS) + (1 | SSB),
              data = diptera_hemiptera)

glmm.hp(dh_m1)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0364/(0.0002+0.0364)
...
dh_m3 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID),
    data = diptera_hemiptera)

glmm.hp(dh_m3)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0555/(0.0001+0.0555)
...
dl_m3 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID),
    data = diptera_lepidoptera)

glmm.hp(dl_m3)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0584/(0.0007+0.0584)
...
dl_m3 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID),
    data = diptera_lepidoptera)

glmm.hp(dl_m3)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0707/(0.0004+0.0707)
...
# remove study
hl_m3 <-lmer(logAbundance ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID),
    data = hemiptera_lepidoptera)

glmm.hp(hl_m3)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0389/(0.0010+0.0389)
...
```{r}
rm(hemiptera_lepidoptera, hl_m3)
```
...
# remove study
hl_m3 <-lmer(logSR ~ Predominant_land_use + Higher_taxon + Predominant_land_use:Higher_taxon + (1 | Source_ID),
    data = hemiptera_lepidoptera)

glmm.hp(hl_m3)

# variance explained by LUI:Order/(variance explained by LUI + variance explained by LUI:Order)
0.0435/(0.0019+0.0435)
