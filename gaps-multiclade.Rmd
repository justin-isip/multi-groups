---
title: "Gaps-in-multiclade"
author: "Justin Isip"
date: "2024-03-22"
output: html_document
---

#### This markdown cleans a PREDICTS diversity extract of bycatch data and then will look at gaps and biases in the data set.
...
#### Calculate number of studies per comparison
...
hemi_dip_ss <-  model_data_ab %>% 
  dplyr::select(SS, Order) %>% 
  filter(Order %in% c("Hemiptera", "Diptera")) %>% 
  unique() %>% 
  group_by(SS) %>% 
  tally() %>% 
  filter(n == 2) %>% 
...
diptera_hemiptera <- model_data_ab %>% filter(SS %in% hemi_dip_ss) %>% 
...
table(diptera_hemiptera$Order, diptera_hemiptera$Predominant_land_use)

# Diptera vs Lepidoptera
diptera_lepidoptera <- model_data_ab %>% filter(SS %in% lep_dip_ss) %>% 
  # then filter the two orders we want to compare, as some studies will have include more than those two orders
  filter(Order %in% c("Diptera", "Lepidoptera")) %>% 
  droplevels()

table(diptera_lepidoptera$Order, diptera_lepidoptera$Predominant_land_use)
...
table(hemiptera_lepidoptera$Order, hemiptera_lepidoptera$Predominant_land_use)

# Hemiptera vs Hymenoptera
hemiptera_hymenoptera <- model_data_ab %>% filter(SS %in% hemi_hym_ss) %>% 
  # then filter the two orders we want to compare, as some studies will have include more than those two orders
  filter(Order %in% c("Hemiptera", "Hymenoptera")) %>% 
  droplevels()

table(hemiptera_hymenoptera$Order, hemiptera_hymenoptera$Predominant_land_use)
